...
public class ScanService {
    ...
    public ScanResult processScan(String badgeUid, PresenceType presenceType) {
        log.info("Scan: {} · {}", badgeUid, presenceType);
        ...
        if (member.isPermanent()) {
            ...
            saveEvent(member, badgeUid, AccessEventType.ENTRY_GRANTED, presenceType, new BigDecimal("1.0"), null);
        ...
        if (member.getMembership() == MembershipType.JOURNEE_ESSAI) {
            ...
            saveEvent(member, badgeUid, AccessEventType.ENTRY_GRANTED, PresenceType.TRIAL, BigDecimal.ZERO, null);
            return new ScanResult.Granted(member, p, null, null);
        }

        int hour = LocalTime.now().getHour();
        if (hour < openH || hour >= closeH) {
            saveEvent(member, badgeUid, AccessEventType.ENTRY_DENIED, null, null, "outside_hours");
        ...
        if (remaining.compareTo(needed) < 0) {
            saveEvent(member, badgeUid, AccessEventType.ENTRY_DENIED, presenceType, null, "pack_exhausted");
            return new ScanResult.Denied("pack_exhausted", member);
        }

        Presence p = savePresence(member, presenceType);
        saveEvent(member, badgeUid, AccessEventType.ENTRY_GRANTED, presenceType, needed, null);
