...
#
# Usage (as root):
#   curl -fsSL https://raw.githubusercontent.com/jreuse-code/amicale/master/ops/bootstrap.sh | bash
#   # or, with a local copy:
#   sudo bash bootstrap.sh
#
# Optional env vars:
#   REPO_URL  — git URL to clone (default: https public clone)
#   APP_DIR   — install directory (default: /opt/amicale)

set -euo pipefail

REPO_URL="${REPO_URL:-https://github.com/jreuse-code/amicale.git}"
...
log "2/7  Docker Engine + compose plugin (official repo)"
...
log "3/7  deploy user + docker group"
if ! id "${DEPLOY_USER}" >/dev/null 2>&1; then
  useradd -m -s /bin/bash "${DEPLOY_USER}"
fi
usermod -aG docker "${DEPLOY_USER}"

log "4/7  UFW (allow 22, 80, 443; deny everything else inbound)"
...
log "5/7  Clone or update repo at ${REPO_DIR}"
